...
const State = {
  tracks: [], currentIndex: -1, playing: false,
  shuffle: false, repeat: false, volume: 0.8,
  sound: null, progressTimer: null, currentView: 'songs'
}
  ...
  for (const filePath of filePaths) {
    try {
      ...
      const track = {
        ...
        cover: null, liked: false, playCount: 0
      }
      if (tag.picture && tag.picture.length > 0) {
        const pic = tag.picture[0]
        track.cover = URL.createObjectURL(new Blob([pic.data], { type: pic.format }))
      }
      State.tracks.push(track)
    } catch (err) { console.error('Error:', filePath, err.message) }
  }
  renderCurrentView()
}
...
function playTrack(index) {
  ...
  if (State.sound) { State.sound.stop(); State.sound.unload(); clearInterval(State.progressTimer) }
  State.currentIndex = index
  const track = State.tracks[index]
  track.playCount++
  const fileUrl = 'file:///' + track.path.replace(/\\/g, '/')
  State.sound = new Howl({
    src: [fileUrl], html5: true, volume: State.volume,
    onplay: () => { State.playing = true; updatePlayButton(); updateNowPlaying(); generateWaveform(); startProgressTimer(); renderCurrentView() },
    onend: () => { clearInterval(State.progressTimer); State.repeat ? playTrack(index) : playNext() },
    onpause: () => { State.playing = false; updatePlayButton(); clearInterval(State.progressTimer) },
...
function playNext() {
  ...
  playTrack(State.shuffle ? Math.floor(Math.random() * State.tracks.length) : (State.currentIndex + 1) % State.tracks.length)
}
...
function generateWaveform() {
  ...
  for (let i = 0; i < bars; i++) {
    const h = 0.15 + Math.abs(Math.sin(i * 0.4) * 0.4 + Math.sin(i * 0.15) * 0.3 + Math.random() * 0.15)
    waveformBars.push(Math.min(h, 0.95))
  }
  drawWaveform(0)
}
...
function drawWaveform(pct) {
  ...
  const barW = W / bars
  const playedUntil = Math.floor(pct * bars)
  for (let i = 0; i < bars; i++) {
    const h = waveformBars[i] * H
    const x = i * barW, y = (H - h) / 2
    ctx.fillStyle = i < playedUntil ? '#888' : '#2a2a2a'
    ctx.beginPath()
    ctx.roundRect(x, y, barW - 1, h, 1)
    ctx.fill()
  }
...
function applyAmbientColor(rgb) {
  ...
  if (!rgb) { panel.style.background = ''; overlay.style.background = ''; return }
...
function updateNowPlaying() {
  ...
  artEl.innerHTML = track.cover ? `<img src="${track.cover}" alt="Carátula">` : `<div id="art-inner">♪</div>`
...
function renderCurrentView() {
  State.currentView === 'favorites' ? renderFavorites() : renderTracklist()
}
...
function renderTracklist() {
  ...
  container.innerHTML = ''
  State.tracks.forEach((track, i) => container.appendChild(buildTrackItem(track, i)))
}
...
function renderFavorites() {
  ...
  container.innerHTML = ''
  favs.forEach(track => container.appendChild(buildTrackItem(track, State.tracks.indexOf(track))))
}
...
document.getElementById('waveform-bar').addEventListener('click', (e) => {
  if (!State.sound) return
  const rect = document.getElementById('waveform-bar').getBoundingClientRect()
  const pct = (e.clientX - rect.left) / rect.width
  State.sound.seek(pct * State.sound.duration())
  drawWaveform(pct)
})
...
document.querySelectorAll('.nav-item').forEach(item => {
  item.addEventListener('click', () => {
    ...
    const titles = { songs: 'Todas las canciones', favorites: 'Favoritas', albums: 'Álbumes', artists: 'Artistas', stats: 'Estadísticas', equalizer: 'Ecualizador', settings: 'Configuración' }
...
document.addEventListener('mousemove', (e) => {
  ...
  if (newWidth >= 220 && newWidth <= 420) panel.style.width = newWidth + 'px'
})
